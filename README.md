...
options:
  ...
  --zeta ZETA           Вес метрики BLIP Caption + BERTScore к prompt. По умолчанию берётся из JSON-конфига или 0.0.
  --pipeline-config PIPELINE_CONFIG
                        Путь к JSON-файлу конфигурации пайплайна. Определяет, какие метрики включены и их веса по
                        умолчанию. Если не указан, используется стандартная конфигурация.
  --chunk CHUNK         Максимальное количество токенов в одном фрагменте текста для SigLIP.
...
```bash
git add .
git commit -m "feat: Добавлена система конфигурации пайплайна через JSON"
git push origin feature/workflow_tune_pipe
```
