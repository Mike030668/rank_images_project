...
DEFAULT_PIPELINE_CONFIG: Dict[str, Any] = {
    "pipeline": {
        "enabled_metrics": ["sig", "flor", "iqa", "dino", "blip_cap"], # <-- Старое
        #"enabled_metrics": ALL_METRICS, # <-- Новое: используем централизованный список
        "default_weights": {
            "alpha": ALPHA_DEFAULT,
            "beta": BETA_DEFAULT,
            "gamma": GAMMA_DEFAULT,
...
def get_all_metrics(config: Dict[str, Any]) -> List[str]:
    ...
    # return list(ALL_METRICS.keys()) # <-- Нужно вернуть ключи словаря
    return list(ALL_METRICS.keys()) # <-- Исправлено
